...
class Layer3Firewall:
    ...
    class FirewallRule:
        ...
        def matches_rule(self, packet):
            src_port_match = self.src_port == None or self.src_port == 0 or self.src_port == packet.src_port
            dest_port_match = self.dest_port == None or self.dest_port == 0 or self.dest_port == packet.dest_port
            src_IP_match = self.src_ip_range == None or self.src_ip_range.ip_is_in_range(packet.src_ip)
            des_IP_match = self.dest_ip_range == None or self.dest_ip_range.ip_is_in_range(packet.dest_ip)
        ...
        def print(self):
            ...
            if self.src_ip_range == None:
                srcIP = "Any"
            else:
                srcIP = self.src_ip_range.to_string()

            if self.dest_ip_range == None:
                desIP = "Any"
            else:
